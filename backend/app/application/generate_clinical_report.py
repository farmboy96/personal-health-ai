...
def _build_docx(
    *,
    ...
    doc = Document()
    sect = doc.sections[0]
    sect.top_margin = Inches(0.7)
    sect.bottom_margin = Inches(0.7)
    ...
    p2.add_run(
        "Patient: male, 54 — prepared from patient-entered data and PubMed literature"
    )
    ...
    p3.add_run(
        "Prepared using AI-assisted synthesis of patient data and PubMed literature."
    )

    doc.add_page_break()

    # Disclaimer box (table)
    doc.add_heading("Disclaimer", level=1)
    tbl_d = doc.add_table(rows=1, cols=1)
    tbl_d.style = "Table Grid"
    cell = tbl_d.rows[0].cells[0]
    disc = (
    ...
    for tr in topic_results:
        ...
        for para in (tr.get("narrative") or "").split("\n\n"):
            if para.strip():
        ...
        for s in studies:
            ...
            stypes = ", ".join(s.get("publication_types") or [])[:200]
            row[0].text = str(s.get("pmid", ""))
            row[1].text = (s.get("title") or "")[:300]
            row[2].text = jy[:200]
            row[3].text = stypes
            row[4].text = str(s.get("relevance_grade", ""))
            row[5].text = str(s.get("importance_grade", ""))
            row[6].text = str(s.get("confidence", ""))
            row[7].text = (s.get("summary") or "")[:800]
            row[8].text = (s.get("applicability_note") or "")[:400]

    doc.add_page_break()
    doc.add_heading("Appendix A — Current regimen (from patient context)", level=1)
    doc.add_paragraph(_extract_regimen_block(USER_CONTEXT))
    ...
    doc.add_paragraph(lt.get("summary_text") or "(none — run daily summary generation first)")

    doc.add_heading("Appendix D — Physiology rollups", level=1)
    doc.add_paragraph(patient_bundle.get("physiology_rollups_text") or "")
